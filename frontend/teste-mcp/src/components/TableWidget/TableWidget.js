...
const TableWidget = ({ deviceId, config }) => {
    ...
    try {
      ...
      const socketHandler = (data) => {
        try {
          ...
          logger.warn('Erro no handler do socket:', e.message);
        }
      };
    ...
    try {
      ...
      if (typeof config === 'string') {
        try {
          ...
          logger.error('Erro ao parsear config JSON:', e.message);
        ...
        const transformedData = response.data.map((item, index) => {
          ...
          if (typeof payload === 'string') {
            try {
              ...
              logger.warn('Erro ao parsear payload:', e.message);
              payload = {};
            }
          }
          ...
          Object.entries(thresholds).forEach(([field, limits]) => {
            ...
            if (value !== undefined && value !== null && value !== '') {
              try {
                ...
                logger.warn('Erro ao processar threshold:', e.message);
              }
            }
          });
      ...
      logger.error('Erro ao buscar excedências:', err.message);
  ...
  const formatDateTime = (timestamp) => {
    ...
    try {
      ...
      logger.warn('formatDateTime error:', e.message);
      return '';
    }
  };
